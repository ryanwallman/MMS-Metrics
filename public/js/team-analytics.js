  ...
  "lib/metricsSourcesRegistry.js"(exports, module) {
    ...
    function parseRegistryCsv(csvText) {
      ...
      for (let i = 1; i < rows.length; i += 1) {
        ...
        if (!key) continue;
        out[key] = key === SOURCE_KEYS.allTimeStats ? publishedCsvUrlFromSheetLink(rawUrl) : browserUrlToCsvFetchUrl(rawUrl);
      }
      return out;
    }
...
// lib/teamAnalytics.js
var require_teamAnalytics = __commonJS({
  "lib/teamAnalytics.js"(exports, module) {
    "use strict";
    ...
    function findScheduleResult(parsedScheduleGames, teamId, iso, opponentTeamId) {
      ...
      const game = (parsedScheduleGames || []).find((g) => {
        ...
        return away === tid && home === oid || away === oid && home === tid;
      });
      if (!game) return null;
    ...
    function buildTeamGamesFromGamelogs(gamelogs, teamCode) {
      ...
      if (!code || !gamelogs?.bySlateKey) return /* @__PURE__ */ new Map();
    ...
    function buildTeamLineupAnalytics({
      ...
      for (const game of games.values()) {
        ...
        const schedule = opponentTeamId ? findScheduleResult(parsedScheduleGames, teamId, game.iso, opponentTeamId) : null;
        const runsForFinal = schedule?.runsFor != null && Number.isFinite(schedule.runsFor) ? schedule.runsFor : runsFor;
        const runsAgainstFinal = schedule?.runsAgainst != null && Number.isFinite(schedule.runsAgainst) ? schedule.runsAgainst : runsAgainst;
        let wins = 0;
        let losses = 0;
        let ties = 0;
      ...
      const buckets = Array.from(bucketMap.values()).sort((a, b) => b.lineupSize - a.lineupSize).map((bucket) => {
        ...
        return {
          ...
          slg: rates.slg,
          ops: rates.ops,
          avgMissingRound: bucket.missingRoundCount > 0 ? bucket.missingRoundSum / bucket.missingRoundCount : null
        };
      });
      return {
    ...
    function buildLeagueLineupRankings({
      ...
      const rowsBySize = /* @__PURE__ */ new Map();
      for (const team of teams || []) {
      ...
      return Array.from(rowsBySize.entries()).sort((a, b) => b[0] - a[0]).map(([lineupSize, rows]) => {
...
// lib/teamAnalyticsPageData.js
var require_teamAnalyticsPageData = __commonJS({
  "lib/teamAnalyticsPageData.js"(exports, module) {
    "use strict";
    var {
      buildTeamCodeById,
      buildCodeToTeamId,
    ...
    var { loadTeamRosters, normalizeScheduleTeamId } = require_teamRosters();
    var { getGamelogs2026CsvUrl, getScheduleUrl } = require_sheetUrls();
    var { load2026StatsByPlayer } = require_stats2026Loader();
    var { fetchCsvText } = require_fetchCsvText();
    var { fetchCsvRows, buildParsedScheduleGames } = require_dfsLeaderboardScoringContext();
    var { buildTeamLineupAnalytics, buildLeagueLineupRankings } = require_teamAnalytics();
    var Papa = require_papaparse_min();
    var SHARED_CACHE_MS = Number(process.env.TEAM_ANALYTICS_SHARED_CACHE_MS) || Number("600000") || 10 * 60 * 1e3;
    var sharedCache = null;
    var sharedCacheExpiresAt = 0;
    var sharedInflight = null;
    function safeText(value) {
      return (value || "").toString().trim();
    }
    ...
    async function loadTeamAnalyticsSharedDataFresh() {
      ...
      const [teams, stats2026ByPlayer, scheduleRows, gamelogCsvText] = await Promise.all([
        loadTeamRosters(),
        load2026StatsByPlayer(),
        fetchCsvRows(scheduleUrl),
        fetchCsvText(await getGamelogs2026CsvUrl())
      ]);
    ...
    async function loadTeamAnalyticsSharedData() {
      ...
      sharedInflight = loadTeamAnalyticsSharedDataFresh().then((result) => {
        sharedCache = result;
    ...
    async function buildTeamAnalyticsPageData2(teamId = null) {
    ...
    module.exports = {
      buildTeamAnalyticsPageData: buildTeamAnalyticsPageData2,
      parseGamelogDataAsOf
    };
  }
});

// client/team-analytics-entry.mjs
var import_teamAnalyticsPageData = __toESM(require_teamAnalyticsPageData(), 1);
async function fetchTeamAnalyticsData(teamId = null) {
  return (0, import_teamAnalyticsPageData.buildTeamAnalyticsPageData)(teamId);
}
if (typeof window !== "undefined") {
  window.MmsTeamAnalytics = { fetchTeamAnalyticsData };
}
export {
  fetchTeamAnalyticsData
};

...
// lib/dfsLineupPageData.js
var require_dfsLineupPageData = __commonJS({
  "lib/dfsLineupPageData.js"(exports, module) {
    ...
    async function buildDfsLineupPageData2({ slateToken, lineupNorms = [] }) {
      ...
      const schedulePayload = scheduleFull.parsedGames?.length ? scheduleFull : { ...scheduleLite, parsedGames: scheduleFull.parsedGames || [] };
      ...
      const token = String(slateToken || activeSlateToken || "").trim().toUpperCase();
      ...
      const prevSlate = slate.viewToken && /^W\d+$/i.test(slate.viewToken) ? resolvePreviousDfsSlate(slate.viewToken, schedulePayload) : null;
      if (prevSlate) {
        ...
        lastWeekPreview = {
          ...
          byNorm,
          hasStats,
          scored: norms.length > 0 ? scoreLineupFromPointsMap(norms, poolByNorm, byNorm) : null
        };
      }
...
// client/dfs-lineup-pool-entry.mjs
var import_dfsLineupPageData = __toESM(require_dfsLineupPageData(), 1);
var import_playerReplacements = __toESM(require_playerReplacements(), 1);
async function loadDfsLineupPool(slateToken, lineupNorms = []) {
  await (0, import_playerReplacements.refreshLivePlayerReplacements)();
  return (0, import_dfsLineupPageData.buildDfsLineupPageData)({ slateToken, lineupNorms });
}

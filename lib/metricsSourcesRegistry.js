...
/** Published-to-web CSV URL (works in browser; export API may require sign-in). */
...
function parseRegistryCsv(csvText) {
  ...
  for (let i = 1; i < rows.length; i += 1) {
    ...
    if (!key) continue;
    out[key] =
      key === SOURCE_KEYS.allTimeStats
        ? publishedCsvUrlFromSheetLink(rawUrl)
        : browserUrlToCsvFetchUrl(rawUrl);
  }
  return out;
}

const REGISTRY_CACHE_TTL_MS =
  Number(process.env.METRICS_SOURCES_CACHE_TTL_MS) || 60 * 1000;

/** Always fetch fresh — registry must not use csvTextCache or sessionStorage (10 min stale). */
async function fetchRegistryCsvText() {
  const base = metricsSourcesRegistryCsvUrl();
  const url = `${base}${base.includes("?") ? "&" : "?"}_=${Date.now()}`;
  const timeoutMs = Number(process.env.CSV_FETCH_TIMEOUT_MS) || 0;
...
const registryCache = createMemoryCache(REGISTRY_CACHE_TTL_MS, "metrics-sources");

...
const { getReplacementsCsvUrl, invalidateSourceCsvCache, SOURCE_KEYS } = require("./sheetUrls");
...
/** Bust CSV + memory cache and reload replacements (live sheet updates). */
...
function replacementsSignature(byOriginalNorm) {
  ...
  const parts = [];
  for (const [norm, entry] of [...byOriginalNorm.entries()].sort((a, b) =>

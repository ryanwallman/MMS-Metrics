...
function getSlatePointsContext(scoringDeps, slate) {
...
function scoreLineupDoc(lineup, slate, slatePointsCtx) {
  ...
  const scored = scoreLineupFromPointsMap(lineup.playerNorms, poolByNorm, pointsByNorm);
...
function buildWeeklyLeaderboardFromLineups(lineups, slate, scoringDeps) {
  ...
  const slatePointsCtx = getSlatePointsContext(scoringDeps, slate);

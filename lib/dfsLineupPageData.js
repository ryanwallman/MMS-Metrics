...
const { remapLineupNorms, filterReplacementsForDate } = require("./playerReplacements");
...
async function buildDfsLineupPageData({ slateToken, lineupNorms = [] }) {
  ...
  const scored =
    showSlateStats && norms.length
      ? scoreLineupFromPointsMap(norms, poolByNorm, slateStats.byNorm)
      : null;

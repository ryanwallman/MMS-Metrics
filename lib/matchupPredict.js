...
/** Win % blend: defense-aware runs, talent, record, and power rankings. */
...
/** Prior games at .500 when regressing team win % (small samples stay near coin flip). */
const MATCHUP_RECORD_PRIOR_GAMES = 6;
/** Logit scale on home − away regressed win % (0.10 gap ≈ moderate favorite). */
const MATCHUP_RECORD_LOGIT_SCALE = 2.5;
/** Logit scale on power-ranking (team overall +/-) gap. */
const MATCHUP_POWER_RANK_LOGIT_SCALE = 0.55;
/** Run-margin curve (~2-run edge ≈ 65% before shrink). Slightly steeper than before so RD spreads win %. */
const MATCHUP_RUN_MARGIN_LOGIT = 0.29;
/** Scale win-% implied margin for run line (1.0 ≈ logistic inverse; >1 widens spreads). */
const MATCHUP_RUN_LINE_WIN_SCALE = 1.05;
/** Shrink win % derived from run line toward 50% (same idea as model shrink). */
const MATCHUP_DISPLAY_WIN_FROM_LINE_SHRINK = 0.5;
...
/** Composite team strength (sum = 1; record/power are separate win-% inputs). */
const MATCHUP_POWER_WEIGHT_OFFENSE = 0.26;
const MATCHUP_POWER_WEIGHT_RUN_PROD = 0.08;
const MATCHUP_POWER_WEIGHT_RUNS_FOR = 0.12;
const MATCHUP_POWER_WEIGHT_RUNS_AGAINST = 0.11;
/** Schedule run differential (RF − RA per game) — separates teams that score and prevent runs. */
const MATCHUP_POWER_WEIGHT_RUN_DIFF = 0.07;
const MATCHUP_POWER_WEIGHT_DEFENSE_Z = 0.08;
const MATCHUP_POWER_WEIGHT_TEAM_OVERALL = 0.28;
const MATCHUP_POWER_WEIGHT_SOS = 0.05;
/** Run projection: offense +/- vs opponent runs-allowed / defensive +/-. */
const MATCHUP_RUN_OFF_Z_PCT = 0.1;
const MATCHUP_RUN_DEF_Z_PCT = 0.12;
const MATCHUP_SCHEDULE_RUNS_BLEND = 0.5;
const MATCHUP_SCHEDULE_DEFENSE_BLEND = 0.45;
...
function teamCompositePower(profile, norms, isHome) {
  ...
  let power =
    ...
    MATCHUP_POWER_WEIGHT_RUN_DIFF * zRd +
    MATCHUP_POWER_WEIGHT_DEFENSE_Z * zDef +
...
/** Win chance from regressed W–L records; low-game samples stay near 50/50. */
function winProbFromRecord(
  homeProfile,
  awayProfile,
  recordLogitScale = MATCHUP_RECORD_LOGIT_SCALE
) {
...
/** Win chance from power-ranking team overall +/- (roster talent + record + SOS). */
function winProbFromPowerRanking(
  homeProfile,
  awayProfile,
  norms,
  logitScale = MATCHUP_POWER_RANK_LOGIT_SCALE
) {
...
function projectRosterExpectedRuns(profile, opponentProfile, norms, runBase, venueFactor) {
  ...
  let mult =
    (1 + MATCHUP_RUN_OFF_Z_PCT * attackBlend) *
    (1 - defenseZScale * defBlend) *
    venueFactor;
...
function scheduleMatchupRunsEstimate(profile, opponentProfile, runBase) {
  ...
  return (
    (1 - MATCHUP_SCHEDULE_DEFENSE_BLEND) * attackEst +
    MATCHUP_SCHEDULE_DEFENSE_BLEND * defenseEst
  );
}
...
function projectTeamRuns(profile, opponentProfile, norms, runBase, venueFactor) {
  ...
  const hasSched =
    scheduleProj != null &&
    ((profile.runsPerGame != null && profile.scheduleGames >= 2) ||
      (opponentProfile.runsAgainstPerGame != null && opponentProfile.scheduleGames >= 2));
...
function enrichMatchupPredictionLines(prediction) {
  ...
  const modelPHome =
    prediction.modelWinPct?.home != null
      ? prediction.modelWinPct.home / 100
      : pHome;
  const runs = {
    away: awayR,
    home: homeR,
...
function matchupFavoriteSide(marginHome, homeWinProb = 0.5) {
  return marginHome > 1e-9 || (Math.abs(marginHome) <= 1e-9 && homeWinProb >= 0.5)
    ? "home"
    : "away";
}
...
function matchupFavoriteRunLineMagnitude(marginHome, modelFavWinProb) {
  ...
  const marginFromWin =
    favoriteRunMarginFromWinProb(modelFavWinProb) * MATCHUP_RUN_LINE_WIN_SCALE;
  // Projected final score sets the spread once it exceeds a half-run; win % only widens pick'ems.
  if (marginProj > 0.5 + 1e-9) return marginProj;
  return Math.max(0.5, marginFromWin);
}

/** Run line from model win % + score margin; displayed win % from final line (monotonic, shrunk). */
function buildMatchupLineAndDisplayWinPct(marginHome, modelHomeWinProb = 0.5) {
  ...
  return {
    ...
    pAway: 1 - pHome,
    runLine: label
      ? { side: homeFavorite ? "home" : "away", value: label }
      : { side: null, value: null },
  };
}
...
function buildFavoriteRunLine(proj, homeWinProb = 0.5) {
  ...
  return buildMatchupLineAndDisplayWinPct(proj.marginHome, homeWinProb).runLine;
}
...
function predictMatchupGame(awayProfile, homeProfile, norms, runBase) {
  ...
  const pHomeRaw =
    ...
    MATCHUP_WIN_WEIGHT_FROM_RECORD * winFromRecord.home +
    MATCHUP_WIN_WEIGHT_FROM_POWER * winFromPower.home;

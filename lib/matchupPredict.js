...
/** Scale win-% implied margin for run line (full inverse logistic is too steep for display). */
const MATCHUP_RUN_LINE_WIN_SCALE = 0.62;
...
function matchupFavoriteRunLineMagnitude(marginHome, favWinProb) {
  ...
  const marginFromWin =
    favoriteRunMarginFromWinProb(favWinProb) * MATCHUP_RUN_LINE_WIN_SCALE;
  let magnitude = Math.max(marginProj, marginFromWin);
  if (magnitude < 1e-9) magnitude = 0.5;
  return magnitude;
}

/** Derive paired run line + win % so larger spreads always mean higher favorite win %. */
...
function buildFavoriteRunLine(proj, homeWinProb = 0.5) {
  ...
  return syncMatchupDisplayWinPctAndRunLine(proj.marginHome, homeWinProb).runLine;
}

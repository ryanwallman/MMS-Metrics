...
/** Offense rating blend for DFS salaries: career + current season (non-rookies). */
const DFS_OFFENSE_RATING_WEIGHT_HISTORICAL = 0.6;
const DFS_OFFENSE_RATING_WEIGHT_2026 = 0.4;
/** 2026 PA needed before a player gets the full current-year share (late starters keep career weight). */
const DFS_SALARY_CURRENT_YEAR_BLEND_FULL_PA = 20;

/**
 * Salary bump when a team has two scheduled games (more plate appearances).
 * Applied to the matchup composite before tiering — not a full 2× price.
 */
const DFS_DOUBLEHEADER_SALARY_BOOST = 1.18;
...
/** Sunday doubleheaders: at most two scheduled games per team on a slate. */
const MAX_SLATE_GAMES_PER_TEAM = 2;
...
/** One line per game; slash between when two differ (field / pitcher / opponent). */
function formatDoubleheaderCellText(labels) {
  const slice = (labels || [])
    .slice(0, MAX_SLATE_GAMES_PER_TEAM)
    .map((l) => safeText(l) || "—");
...
function averageOpposingPitcherTooltip(matchups, teams) {
  const pits = matchups
    .slice(0, MAX_SLATE_GAMES_PER_TEAM)
    .map((m) => pitcherForTeamId(m.opponentId, teams))
    .filter(Boolean);
  ...
  return { baa: Math.round(baa * 1000) / 1000, runsG: Math.round(runsG * 100) / 100 };
}
...
/**
 * Doubleheader scoring: sum each game’s fantasy points, divide by games played,
 * round to nearest whole number (e.g. 10 pts + 0 pts → 5, not 5.0).
 */
...
/** Slate that just locked: option immediately before the open editable slate (or last locked). */
...
/** First schedule slate whose game day is today or still in the future (calendar order). */
...
/** Default matchup predictor view: open DFS slate, else next upcoming game day, else last locked. */
...
/** Past = locked slates; future = open + not-yet-locked slates. */
...
function filterScheduleOptionsForMatchupPredictorMode(
  scheduleOptions,
  schedulePayload,
  refIso,
  nowMs,
...
/** Matchup predictor date picker: full season (all past + all future slates). */
function filterScheduleOptionsForMatchupPredictor(scheduleOptions) {
  return scheduleOptions || [];
}

/** Limit date picker to DFS-visible slates (past + current open week). */
function filterScheduleOptionsToDfsVisibility(scheduleOptions, schedulePayload, refIso, nowMs = Date.now()) {
  const visible = new Set(
    filterVisibleDfsSlateOptions(buildDfsSlateOptions(schedulePayload, refIso, nowMs)).map((o) =>
      safeText(o.value).toUpperCase()
    )
  );
...
function buildDfsPlayerPool({
  ...
  for (const t of teams) {
    ...
    const gameLabel = matchups
      .map((m) => (m.game ? `${m.game.away} @ ${m.game.home}` : ""))
      .filter(Boolean)
      .join(" · ");
...
const EMPTY_GAMELOGS = { byNorm: new Map(), bySlateKey: new Map(), gameIsos: new Set() };
...
/** Default leaderboard week: most recently locked DFS slate. */

...
/**
 * Sunday week (W#) uses calendar-based `isPast` from {@link buildWeekSlateFromToken}.
 * Wednesday (D########) uses the same lock/edit semantics as the lineup builder via {@link buildSlateFromToken}.
 */
function buildLeaderboardSlateFromToken(
  ...
  const v = safeText(viewToken).toUpperCase();
  if (/^W\d+$/.test(v)) {
    return buildWeekSlateFromToken(v, schedulePayload, refIso);
  }
  if (/^D\d{8}$/.test(v)) {

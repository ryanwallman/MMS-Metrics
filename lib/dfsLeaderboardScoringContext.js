...
const { parsePlayerHistoricalStatsCsv } = require("./playerHistoricalStats");
...
const { extendOffenseRatingsForReplacements } = require("./offenseRankingsPage");

async function load2025HistoricalByPlayer() {
  /** Superseded by live all-time stats (metrics_sources B10). Kept for API compatibility. */
  return new Map();
}
...
async function loadCareerByPlayer() {
  ...
  if (src.type === "file") {
    ...
    const legacyStatic =
      overrideUrl === "/data/csv/career.csv" ||
      overrideUrl.endsWith("/data/csv/career.csv");
    const url =
      overrideUrl && !legacyStatic
        ? overrideUrl
        : await getAllTimeStatsCsvUrl();
    try {

...
const { buildTeamCodeById, load2026GamelogsByPlayer, normalizePlayerName } = require("./dfs");
const {
  INDEX_URL,
  SCHEDULE_URL,
  ROSTER_URL,
...
async function loadTeamRosters() {
  ...
  for (let id = 1; id <= 18; id += 1) {
    ...
    const players =
      canonicalRostersByTeamId[teamId] || rosterByCaptain.get(teamMeta.captain) || [];
...
function blendedOffenseRating(composite26, compositeHist, has26, hasHist) {
  if (has26 && hasHist) {
    return (
      OFFENSE_RATING_WEIGHT_HISTORICAL * compositeHist + OFFENSE_RATING_WEIGHT_2026 * composite26
    );
  }

...
const { buildRosterEntriesWithReplacements } = require("./playerReplacements");
...
function enrichRosterForMatchupView(
  ...
  const entries = byOriginalNorm?.size
...
function playerRatingWeight(rating) {
  ...
  return Math.max(0.35, Math.min(2.1, 0.65 + r * 0.55));
}
...
/** Mean offensive +/- across the full roster (active + missing). */
...
/** Per-player bench sensitivity vs team average (kept moderate for one sit). */
const BENCH_VS_AVG_SENSITIVITY = 0.68;

/**
 * One benched player vs team average: gap = rating − avg (positive = good player sat).
 * exp(−gap × k) compounds when multiple players sit — two stars hurt more than 2× one star.
 */
function benchPlayerFactorVsTeamAvg(missingRating, teamAvg) {
  const gap = missingRating - teamAvg;
  return Math.exp(-gap * BENCH_VS_AVG_SENSITIVITY);
}

/** @deprecated alias — positive gap above avg still hurts via benchPlayerFactorVsTeamAvg */
function missingRatingFactorVsTeamAvg(missingRating, teamAvg) {
  return benchPlayerFactorVsTeamAvg(missingRating, teamAvg);
}

/** Identity-only bench swing is damped for one player, stronger as more sit. */
...
/** Legacy helper (tests / diagnostics). Positive = good player missing hurts. */
 ...
 * ≥10 fielders: solely player rating vs team average (bad out → boost, good out → penalty).
 * <10 fielders: missing a fielder always hurts; lower-rated missing players soften the hit.
 */
...
function applyMissingPlayersToProfile(
  ...
  const adjustedDefense =
    teamMult.regime === "lineup-adjust"
      ? anchorDef * rosterDefFactor
      : rosterDef * teamMult.defense;

  const baseTeam = baseProfile.teamOverall;
  const adjustedTeamOverall =
    baseTeam != null && Number.isFinite(baseTeam) && anchorOff > 0
      ? baseTeam * rosterOffFactor * offenseMult
      : baseTeam;
  ...
  return {
    ...
    lineupAlerts,
    teamMultiplier:
      anchorOff > 0 ? Math.max(0.01, adjustedOffense / anchorOff) : teamMult.offense,

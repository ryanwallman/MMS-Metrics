...
var require_offenseRankingsPage = __commonJS({
  "lib/offenseRankingsPage.js"(exports, module) {
    ...
    function historicalPaAndBundleForPlayer(normalizedKey, careerByPlayer, _hist2025ByPlayer) {
      const c = careerByPlayer.get(normalizedKey);
      if (c && toNumber(c.pa) > 0) {
        const pa = toNumber(c.pa);
        const bundle = computeOffenseRateBundle(pa, c.ab, c.bb, c.h, c.tb, c.r, c.rbi);
...
// lib/plusMinusPage.js
var require_plusMinusPage = __commonJS({
  "lib/plusMinusPage.js"(exports, module) {
    "use strict";
    var { loadTeamRosters } = require_teamRosters();
    var { load2026StatsByPlayer } = require_stats2026Loader();
    var { getCachedPlayerReplacements } = require_playerReplacements();
    ...
    var {
      ...
      weightedMomentsPerMetric,
      buildOffensivePlayerRows,
      buildSinglePlayerRatingRow
    } = require_offenseRankingsPage();
    function buildPlusMinusRows({
      teams,
      careerByPlayer,
      hist2025ByPlayer,
      stats2026ByPlayer,
      ...
      const rosterRows = buildOffensivePlayerRows(
        teams,
        careerByPlayer,
        hist2025ByPlayer,
        stats2026ByPlayer,
      ...
      const teamByOriginalNorm = /* @__PURE__ */ new Map();
      const rosterTeamByNorm = /* @__PURE__ */ new Map();
      for (const row of rosterRows) {
        teamByOriginalNorm.set(row.norm, { teamId: row.teamId, teamName: row.teamName });
        rosterTeamByNorm.set(row.norm, { teamId: row.teamId, teamName: row.teamName });
      }
      for (const entry of byOriginalNorm?.values() || []) {
        const replTeam = rosterTeamByNorm.get(entry.replacementNorm);
        if (replTeam && !teamByOriginalNorm.has(entry.originalNorm)) {
          teamByOriginalNorm.set(entry.originalNorm, replTeam);
        }
      }
      const replacementByNorm = /* @__PURE__ */ new Map();
      for (const entry of byOriginalNorm?.values() || []) {
        replacementByNorm.set(entry.replacementNorm, entry);
      }
      const rows = rosterRows.map((row) => {
        const repl = byOriginalNorm?.get(row.norm);
        const isReplacementOnRoster = replacementByNorm.has(row.norm);
        const replEntry = isReplacementOnRoster ? replacementByNorm.get(row.norm) : null;
        if (repl) {
          return {
            ...row,
            replacementRole: "replaced",
            relatedPlayer: repl.replacement,
            replacementDateIso: repl.replacementDateIso || null
          };
        }
        if (replEntry) {
          return {
            ...
            relatedPlayer: replEntry.original,
            replacementDateIso: replEntry.replacementDateIso || null
          };
        }
        return { ...row, replacementRole: null, relatedPlayer: null, replacementDateIso: null };
      });
      const rosterNorms = new Set(rosterRows.map((r) => r.norm));
      for (const entry of byOriginalNorm?.values() || []) {
        if (rosterNorms.has(entry.replacementNorm)) continue;
        const team = teamByOriginalNorm.get(entry.originalNorm) || {
          teamId: "",
          teamName: "\u2014"
        };
        rows.push({
          ...buildSinglePlayerRatingRow({
            playerName: entry.replacement,
            norm: entry.replacementNorm,
            teamId: team.teamId,
            teamName: team.teamName,
            careerByPlayer,
            hist2025ByPlayer,
          ...
          replacementRole: "replacement",
          relatedPlayer: entry.original,
          replacementDateIso: entry.replacementDateIso || null
        });
      }
      for (const entry of byOriginalNorm?.values() || []) {
        if (rosterNorms.has(entry.originalNorm)) continue;
        const team = teamByOriginalNorm.get(entry.originalNorm) || {
          teamId: "",
          teamName: "\u2014"
        };
        rows.push({
          ...buildSinglePlayerRatingRow({
            playerName: entry.original,
            norm: entry.originalNorm,
            teamId: team.teamId,
            teamName: team.teamName,
            careerByPlayer,
            hist2025ByPlayer,
          ...
          replacementRole: "replaced",
          relatedPlayer: entry.replacement,
          replacementDateIso: entry.replacementDateIso || null
        });
      }
      rows.sort((a, b) => b.rating - a.rating);
    ...
    async function buildPlusMinusPageData2() {
      const [teams, careerByPlayer, hist2025ByPlayer, stats2026ByPlayer, replacements] = await Promise.all([
        loadTeamRosters(),
        loadCareerByPlayer(),
        load2025HistoricalByPlayer(),
        load2026StatsByPlayer(),
      ...
      const { moments, totPa } = weightedMomentsPerMetric(bundles);
      const rows = buildPlusMinusRows({
        teams,
        careerByPlayer,
        hist2025ByPlayer,
        stats2026ByPlayer,
        byOriginalNorm: replacements?.byOriginalNorm ?? /* @__PURE__ */ new Map(),
        moments
      });
      return {
        ...
        replacementPairCount: replacements?.byOriginalNorm?.size ?? 0,
        ratingWeights: {
          historical: OFFENSE_RATING_WEIGHT_HISTORICAL,
          current2026: OFFENSE_RATING_WEIGHT_2026
        }
      };
    }
    module.exports = { buildPlusMinusPageData: buildPlusMinusPageData2, buildPlusMinusRows };
  }
});

// client/plus-minus-entry.mjs
var import_plusMinusPage = __toESM(require_plusMinusPage(), 1);
async function fetchPlusMinusData() {
  return (0, import_plusMinusPage.buildPlusMinusPageData)();
}
if (typeof window !== "undefined") {
  window.MmsPlusMinus = { fetchPlusMinusData };
}
export {
  fetchPlusMinusData
};
